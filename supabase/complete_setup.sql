-- Полная схема одним файлом (для пустого проекта в Supabase → SQL Editor).
-- Либо применяйте по очереди файлы из migrations/, либо выполните этот скрипт целиком.
...
-- Апгрейд со старой схемы без колонок (безопасно на новой БД)
